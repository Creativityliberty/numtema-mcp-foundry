...
export function verifySignedArtifact(
  ...
  const errors: SignatureVerificationResult['errors'] = [];
  const atMs = Date.parse(at);
  if (Number.isNaN(atMs)) errors.push({ code: 'INVALID_PREFLIGHT_TIME', message: 'Preflight time must be RFC 3339.' });
  ...
  if (key.purpose !== purpose) errors.push({ code: 'TRUST_KEY_PURPOSE_MISMATCH', message: `Key ${key.key_id} is not trusted for ${purpose}.` });
  if (key.status !== 'active') errors.push({ code: 'TRUST_KEY_INACTIVE', message: `Key ${key.key_id} is ${key.status}.` });
  if (key.not_before !== undefined && atMs < Date.parse(key.not_before)) errors.push({ code: 'TRUST_KEY_NOT_YET_VALID', message: `Key ${key.key_id} is not valid yet.` });
  if (key.expires_at !== undefined && atMs >= Date.parse(key.expires_at)) errors.push({ code: 'TRUST_KEY_EXPIRED', message: `Key ${key.key_id} has expired.` });
  ...
  if (!validSignature) errors.push({ code: 'SIGNATURE_INVALID', message: 'Artifact signature is invalid.' });
