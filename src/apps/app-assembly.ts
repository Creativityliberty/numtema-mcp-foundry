...
import { createMcpRouter, type McpRouter } from '../mcp/jsonrpc-router.js';
import { loadProviderAdapterBundle, parseMcpRuntimeConfig } from '../mcp/config-loader.js';
import type { McpRuntimeConfig } from '../mcp/runtime-config.js';
...
export async function loadChatGptAppAssembly(configPath: string, environment: Record<string, string | undefined> = process.env): Promise<LoadedChatGptAppAssembly> {
  ...
  const runtimeConfig = parseMcpRuntimeConfig(JSON.parse(await readFile(runtimeConfigPath, 'utf8')) as unknown);
  const runtimePath = (value: string): string => resolve(runtimeRoot, value);
...
export function createAppToolCaller(
  runtimeCall: (name: string, args: Record<string, unknown>, meta?: Record<string, unknown>, context?: import('../mcp/types.js').McpRequestContext) => Promise<import('../mcp/types.js').McpCallToolResult>,
  ...
  return async (name, args, meta, context) => {
    if (name.startsWith('foundry_approval_')) return approvalCall(name, args, context);
...
export function createScopeResolver(registry: McpToolRegistry, baselineScopes: string[]): (message: unknown) => string[] {
  return (message) => {
    if (!isRecord(message) || typeof message.method !== 'string') return baselineScopes;
    if (message.method === 'resources/list' || message.method === 'resources/read') return ['mcp:resources'];
    if (message.method === 'tools/list') return ['mcp:tools'];
    if (message.method !== 'tools/call') return baselineScopes;
    const params = isRecord(message.params) ? message.params : {};
    const name = typeof params.name === 'string' ? params.name : '';
    if (name.startsWith('foundry_approval_')) return ['mcp:approve'];
    const tool = registry.get(name);
    return [...new Set(['mcp:tools', ...(tool?.descriptor._meta.requiredScopes ?? [])])].sort();
  };
}

async function loadArtifactMap<T>(entries: Record<string, string> | undefined, root: string, loader: (path: string) => Promise<T>): Promise<Record<string, T>> {
