...
export async function runChatGptAppSmoke(assembly: LoadedChatGptAppAssembly, input: { username: string; password: string; workspaceRef: string; toolName?: string; args?: Record<string, unknown> }): Promise<Record<string, unknown>> {
  const client = assembly.config.oauth.clients[0];
  if (!client) throw new Error('APP_SMOKE_CLIENT_MISSING');
  ...
  if (input.toolName) called = await assembly.router.handle({ jsonrpc: '2.0', id: 4, method: 'tools/call', params: { name: input.toolName, arguments: input.args ?? {}, _meta: { idempotencyKey: `app-smoke-${Date.now()}` } } }, context);
