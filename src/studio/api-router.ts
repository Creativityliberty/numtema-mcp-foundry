...
export async function handleStudioApi(request: IncomingMessage, pathname: string, context: StudioApiContext): Promise<StudioApiResult | null> {
  ...
  try {
    ...
    if (method === 'GET' && pathname === '/api/project') {
      ...
      let capabilityMap: unknown = null;
      try { capabilityMap = await readStudioGeneratedArtifact(context.projectDirectory, 'capability_map'); } catch { /* Not built yet. */ }
    ...
    if (method === 'POST' && pathname === '/api/project/configure') {
      ...
      if (isRecord(payload.provider)) {
        if (typeof payload.provider.base_url === 'string') project.provider.base_url = payload.provider.base_url;
        if (typeof payload.provider.provider_ref === 'string') project.provider.provider_ref = payload.provider.provider_ref;
        if (typeof payload.provider.provider_account_ref === 'string') project.provider.provider_account_ref = payload.provider.provider_account_ref;
        if (typeof payload.provider.credential_environment === 'string') project.provider.credential_environment = payload.provider.credential_environment;
        if (isAuthMode(payload.provider.auth_mode)) project.provider.auth_mode = payload.provider.auth_mode;
      }
      if (isRecord(payload.chatgpt_app)) {
        if (typeof payload.chatgpt_app.public_base_url === 'string') project.chatgpt_app.public_base_url = payload.chatgpt_app.public_base_url;
        if (Array.isArray(payload.chatgpt_app.allowed_origins)) project.chatgpt_app.allowed_origins = payload.chatgpt_app.allowed_origins.filter((item): item is string => typeof item === 'string');
      }
    ...
    if (method === 'POST' && pathname === '/api/overrides') {
      const payload = await readJsonBody(request, context.bodyLimitBytes);
      if (!Array.isArray(payload.overrides)) return fail(400, 'STUDIO_OVERRIDES_INVALID', 'overrides must be an array.');
    ...
    if (method === 'POST' && pathname === '/api/simulate') {
      ...
      const project = await loadStudioProject(context.projectDirectory);
      const adapters = JSON.parse(await readFile(resolveStudioPath(context.projectDirectory, project.paths.provider_adapters), 'utf8')) as ProviderAdapterBundle;
...
function errorCode(error: unknown): string { const message = errorMessage(error); const match = /^([A-Z0-9_]+):/.exec(message); return match?.[1] ?? 'STUDIO_REQUEST_FAILED'; }
function traceId(): string { return `studio_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 10)}`; }
