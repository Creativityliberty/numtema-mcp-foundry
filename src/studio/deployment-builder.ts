...
export async function buildDeploymentPackage(directory: string): Promise<DeploymentPackageManifest> {
  ...
  const manifest: DeploymentPackageManifest = {
...
async function copyDirectory(source: string, destination: string, excludedRoot?: string): Promise<void> {
  await mkdir(destination, { recursive: true });
  const excluded = excludedRoot ? resolve(excludedRoot) : undefined;
  for (const name of await readdir(source)) {
    const from = join(source, name);
    const absoluteFrom = resolve(from);
    if (excluded && (absoluteFrom === excluded || absoluteFrom.startsWith(`${excluded}/`) || absoluteFrom.startsWith(`${excluded}\\`))) continue;
    const to = join(destination, name);
    const info = await stat(from);
    if (info.isDirectory()) await copyDirectory(from, to, excludedRoot);
    else if (info.isFile()) { await mkdir(dirname(to), { recursive: true }); await copyFile(from, to); }
  }
}

async function inventory(root: string, ignored: string[]): Promise<DeploymentPackageManifest['files']> {
  const entries: DeploymentPackageManifest['files'] = [];
...
function coolifyGuide(project: Awaited<ReturnType<typeof loadStudioProject>>): string {
...
function vpsGuide(project: Awaited<ReturnType<typeof loadStudioProject>>): string {
...
function chatGptGuide(project: Awaited<ReturnType<typeof loadStudioProject>>): string {
  const base = project.chatgpt_app.public_base_url.replace(/\/$/, '');
