...
export function createMcpToolRegistry(
  ...
  extraTools: McpRegisteredTool[] = []
): McpToolRegistry {
  const adapterByTool = new Map(adapters.adapters.map((adapter) => [adapter.tool_id, adapter] as const));
  const registrations = [...bundle.tools
    .map((tool): McpRegisteredTool => ({
      descriptor: descriptorFromTool(tool),
      toolId: tool.id,
      adapterId: adapterByTool.get(tool.id)?.id ?? null
    })), ...extraTools]
  ...
  return {
    list(cursor?: string) {
      ...
      return next < registrations.length
        ? { tools: page, nextCursor: encodeCursor(next) }
        : { tools: page };
    },
    get(name: string) {
