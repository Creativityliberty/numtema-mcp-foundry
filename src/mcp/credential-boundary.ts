...
export function injectCredential(
  ...
  binding: ProviderAuthBindingContract | undefined,
  options: CredentialBoundaryOptions
): ProviderExecutionPlan['request'] {
  const request = {
    ...plan.request,
  ...
  if (selected === null || binding === undefined || binding.injection.strategy === 'none') return request;
  const envName = options.credentialEnvironment[selected.credential_handle];
  if (envName === undefined) throw new Error(`CREDENTIAL_ENV_NOT_CONFIGURED: no environment variable is mapped for credential handle ${selected.credential_handle}.`);
  const secret = options.environment[envName];
  if (secret === undefined || secret.length === 0) throw new Error(`CREDENTIAL_MATERIAL_UNAVAILABLE: environment variable ${envName} is empty or missing.`);
  ...
  if (binding.injection.location === 'header') {
    if (name === null) throw new Error('CREDENTIAL_BINDING_INVALID: header injection requires a name.');
  ...
  if (binding.injection.location === 'query') {
    if (name === null) throw new Error('CREDENTIAL_BINDING_INVALID: query injection requires a name.');
  ...
  if (binding.injection.location === 'cookie') {
    if (name === null) throw new Error('CREDENTIAL_BINDING_INVALID: cookie injection requires a name.');
