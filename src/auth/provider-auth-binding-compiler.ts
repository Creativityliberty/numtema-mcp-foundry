...
import type {
  ...
  ProviderAuthBindingBundle,
  ProviderAuthBindingContract
} from './types.js';

export function compileProviderAuthBindings(
  ...
  const warnings: ProviderAuthBindingBundle['warnings'] = [];
  ...
  const payload = {
    artifact_type: 'provider_auth_binding_bundle' as const,
...
function compileBinding(auth: AuthContract, adapters: ProviderAdapterBundle): ProviderAuthBindingContract {
  ...
  const bindingBase = {
    ...
    binding_dimensions: uniqueSorted(auth.credential_binding_dimensions ?? []) as ProviderAuthBindingContract['binding_dimensions'],
    token_passthrough: false as const,
    audience: {
...
function resolveInjection(auth: AuthContract): ProviderAuthBindingContract['injection'] {
  const extension = providerBindingExtension(auth);
  const defaultLocation: AuthInjectionLocation = 'header';
  const defaults: Record<AuthContract['mode'], {
...
function providerBindingExtension(auth: AuthContract): Record<string, unknown> {
  const extensions = auth.extensions;
  if (!isRecord(extensions) || !isRecord(extensions.provider_binding)) return {};
  return extensions.provider_binding;
}

function readLocation(value: unknown): AuthInjectionLocation | null {
