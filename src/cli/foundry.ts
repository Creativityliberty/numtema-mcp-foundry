...
import type { ProviderAdapterBundle, ProviderExecutionPlan } from '../adapters/types.js';
...
import type { CredentialResolutionPlan, ProviderAuthBindingBundle } from '../auth/types.js';
...
import type { CompilationResult } from '../compiler/types.js';
...
import type { CapabilityMapArtifact, SourceInspectionArtifact } from '../inspection/types.js';
import { mapCapabilities } from '../mapping/capability-mapper.js';
import { validateBundle } from '../validation/validate-bundle.js';
import type { ValidationReport } from '../validation/issues.js';
import { authorizeExecution } from '../runtime/secure-preflight.js';
import {
  loadApprovalProof,
  loadBudgetAuthorization,
  loadCredentialResolutionPlan,
  loadProviderExecutionPlan,
  loadRuntimePolicyDecision,
  loadRuntimeTrustStore,
  loadAuthorizedExecutionEnvelope,
  loadSignedDispatchReceipt
} from '../runtime/loaders.js';
import type { AuthorizedExecutionEnvelope } from '../runtime/types.js';
import {
  commitDispatch,
...
import type { DispatchMutationResult, SignedDispatchReceipt } from '../dispatch/types.js';
...
import type { ChatGptAppSummary } from '../apps/app-config.js';
...
interface ParsedCompileArguments extends ParsedArtifactArguments {
  schemaDirectory: string;
}
...
interface ParsedAuthPlanArguments {
  filePath: string;
  toolName: string;
...
export async function runCli(args: string[], io: CliIo = defaultIo): Promise<number> {
  ...
  if (command === 'doctor') return runDoctor(args.slice(1), io);
  if (command === 'demo') return runDemo(args.slice(1), io);
  if (command === 'init') return runInit(args.slice(1), io);
  if (command === 'validate') return runValidate(args.slice(1), io);
  if (command === 'inspect') return runInspect(args.slice(1), io);
  if (command === 'map') return runMap(args.slice(1), io);
  if (command === 'compile') return runCompile(args.slice(1), io);
  if (command === 'adapters') return runAdapters(args.slice(1), io);
  if (command === 'plan') return runPlan(args.slice(1), io);
  if (command === 'auth-bindings') return runAuthBindings(args.slice(1), io);
  if (command === 'auth-plan') return runAuthPlan(args.slice(1), io);
  if (command === 'preflight') return runPreflight(args.slice(1), io);
  if (command === 'ledger') return runLedger(args.slice(1), io);
  if (command === 'dispatch') return runDispatch(args.slice(1), io);
  if (command === 'mcp') return runMcp(args.slice(1), io);
  if (command === 'app') return runApp(args.slice(1), io);
  if (command === 'studio') return runStudio(args.slice(1), io);
...
async function runValidate(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedValidateArguments;
  try {
...
async function runInspect(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedArtifactArguments;
  try {
...
async function runMap(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedArtifactArguments;
  try {
...
async function runCompile(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedCompileArguments;
  try {
...
async function runAdapters(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedArtifactArguments;
  try {
...
async function runPlan(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedPlanArguments;
  try {
  ...
  try {
    ...
    if (!adapter) throw new Error(`UNKNOWN_TOOL: ${parsed.toolName}`);
    const rawArguments = JSON.parse(await readFile(resolve(parsed.argsFile), 'utf8')) as unknown;
    if (!isRecord(rawArguments)) throw new Error('INVALID_ARGUMENTS: arguments file root must be an object.');
...
async function runAuthBindings(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedArtifactArguments;
  try {
...
async function runAuthPlan(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedAuthPlanArguments;
  try {
  ...
  try {
    ...
    const adapter = adapters.adapters.find((candidate) => candidate.tool_name === parsed.toolName || candidate.tool_id === parsed.toolName);
    if (!adapter) throw new Error(`UNKNOWN_TOOL: ${parsed.toolName}`);
    if (adapter.credential.auth_ref === null) throw new Error(`AUTH_NOT_REQUIRED: ${parsed.toolName}`);
    ...
    if (!binding) throw new Error(`AUTH_BINDING_NOT_FOUND: ${adapter.credential.auth_ref}`);
...
async function runPreflight(args: string[], io: CliIo): Promise<number> {
  let parsed: ParsedPreflightArguments;
  try {
  ...
  try {
    ...
    const input: Parameters<typeof authorizeExecution>[0] = { executionPlan, credentialPlan, policy, trustStore, at: parsed.at };
    if (parsed.approvalFile !== null) input.approval = await loadApprovalProof(parsed.approvalFile);
    if (parsed.budgetFile !== null) input.budget = await loadBudgetAuthorization(parsed.budgetFile);
...
async function runLedger(args: string[], io: CliIo): Promise<number> {
  const subcommand = args[0];
  try {
    if (subcommand === 'init') {
      const directory = args[1];
      if (!directory || directory.startsWith('--')) throw new Error('ledger directory is required.');
      ...
      for (let index = 2; index < args.length; index += 1) {
        const argument = args[index]!;
        if (argument === '--json') { json = true; continue; }
        if (argument === '--at') { at = requireOptionValue(args, index, '--at'); index += 1; continue; }
        if (argument === '--ledger-id') { ledgerId = requireOptionValue(args, index, '--ledger-id'); index += 1; continue; }
    ...
    if (subcommand === 'status') {
      ...
      for (let index = 1; index < args.length; index += 1) {
        const argument = args[index]!;
        if (argument === '--json') { json = true; continue; }
        if (argument === '--ledger') { directory = requireOptionValue(args, index, '--ledger'); index += 1; continue; }
        if (argument === '--out') { outFile = requireOptionValue(args, index, '--out'); index += 1; continue; }
        throw new Error(`unknown option ${argument}.`);
      }
      if (directory === null) throw new Error('--ledger is required.');
      const snapshot = await readDispatchLedgerSnapshot(directory);
      const serialized = `${JSON.stringify(snapshot, null, 2)}\n`;
      if (outFile !== null) await writeFile(resolve(outFile), serialized, 'utf8');
      if (json) io.out(serialized);
      else io.out(`LEDGER_STATUS ${snapshot.ledger_id}\n- sequence: ${snapshot.sequence}\n- reservations: ${snapshot.reservations.length}\n- active budget bindings: ${snapshot.budget_totals.filter((entry) => entry.active_amount > 0).length}${outFile === null ? '' : `\nWROTE ${outFile}`}`);
      return 0;
    }
...
async function runDispatch(args: string[], io: CliIo): Promise<number> {
  const subcommand = args[0];
  try {
    if (subcommand === 'verify') {
      let receiptFile: string | null = null;
      let trustStoreFile: string | null = null;
      let at: string | null = null;
      let json = false;
      for (let index = 1; index < args.length; index += 1) {
        const argument = args[index]!;
        if (argument === '--json') { json = true; continue; }
        if (argument === '--receipt') { receiptFile = requireOptionValue(args, index, '--receipt'); index += 1; continue; }
        if (argument === '--trust-store') { trustStoreFile = requireOptionValue(args, index, '--trust-store'); index += 1; continue; }
        if (argument === '--at') { at = requireOptionValue(args, index, '--at'); index += 1; continue; }
        throw new Error(`unknown option ${argument}.`);
      }
      if (receiptFile === null) throw new Error('--receipt is required.');
      if (trustStoreFile === null) throw new Error('--trust-store is required.');
      if (at === null) throw new Error('--at is required.');
    ...
    let signingKeyFile: string | null = null;
    let keyId: string | null = null;
    let at: string | null = null;
    let reason: 'cancelled' | 'expired' | null = null;
    let json = false;
    let outFile: string | null = null;
    for (let index = 1; index < args.length; index += 1) {
      const argument = args[index]!;
      if (argument === '--json') { json = true; continue; }
      if (argument === '--ledger') { ledger = requireOptionValue(args, index, '--ledger'); index += 1; continue; }
      if (argument === '--envelope') { envelopeFile = requireOptionValue(args, index, '--envelope'); index += 1; continue; }
      if (argument === '--reservation') { reservationId = requireOptionValue(args, index, '--reservation'); index += 1; continue; }
      if (argument === '--signing-key') { signingKeyFile = requireOptionValue(args, index, '--signing-key'); index += 1; continue; }
      if (argument === '--key-id') { keyId = requireOptionValue(args, index, '--key-id'); index += 1; continue; }
      if (argument === '--at') { at = requireOptionValue(args, index, '--at'); index += 1; continue; }
      if (argument === '--reason') {
        const value = requireOptionValue(args, index, '--reason');
        if (value !== 'cancelled' && value !== 'expired') throw new Error('--reason must be cancelled or expired.');
        reason = value; index += 1; continue;
      }
      if (argument === '--out') { outFile = requireOptionValue(args, index, '--out'); index += 1; continue; }
      throw new Error(`unknown option ${argument}.`);
    }
    if (ledger === null) throw new Error('--ledger is required.');
    if (signingKeyFile === null) throw new Error('--signing-key is required.');
    if (keyId === null) throw new Error('--key-id is required.');
    if (at === null) throw new Error('--at is required.');
    const signingKeyPem = await readFile(resolve(signingKeyFile), 'utf8');
    let result: DispatchMutationResult;
    if (subcommand === 'reserve') {
      if (envelopeFile === null) throw new Error('--envelope is required.');
      ...
      if (reservationId === null) throw new Error('--reservation is required.');
...
async function runMcp(args: string[], io: CliIo): Promise<number> {
  const subcommand = args[0];
  ...
  if (subcommand === 'init') {
    ...
    for (let index = 1; index < args.length; index += 1) {
      const argument = args[index]!;
      if (argument === '--force') { force = true; continue; }
      if (argument.startsWith('--')) { io.error(`USAGE_ERROR: unknown option ${argument}.`); return 2; }
      directory = argument;
    }
    try {
      ...
      return 0;
    } catch (error) { io.error(`MCP_INIT_ERROR: ${errorMessage(error)}`); return 2; }
  }
  ...
  for (let index = 1; index < args.length; index += 1) {
    const argument = args[index]!;
    if (argument === '--json') { json = true; continue; }
    if (argument === '--config') { configFile = requireOptionValue(args, index, '--config'); index += 1; continue; }
    if (argument === '--tool') { toolName = requireOptionValue(args, index, '--tool'); index += 1; continue; }
    if (argument === '--args') { argsFile = requireOptionValue(args, index, '--args'); index += 1; continue; }
    io.error(`USAGE_ERROR: unknown option ${argument}.`);
    return 2;
  }
  if (configFile === null) {
  ...
  try {
    ...
    if (subcommand === 'smoke') {
      ...
      if (toolName !== null) {
        const toolArgs = argsFile === null ? {} : JSON.parse(await readFile(resolve(argsFile), 'utf8')) as Record<string, unknown>;
      ...
      if (called && typeof called === 'object' && 'result' in called) {
        const result = (called as { result: { isError?: boolean } }).result;
    ...
    await new Promise<void>((resolvePromise) => server.listen(assembly.http.port, assembly.http.host, resolvePromise));
...
function renderMcpSummary(summary: { server_name: string; server_version: string; protocol_version: string; transport: string; tool_count: number; adapter_count: number; authenticated_tool_count: number; high_risk_tool_count: number; provider_base_url: string; ledger_directory: string }): string {
  return [
...
function renderMcpSmoke(report: { listed: unknown; called: unknown; network_call_requested: boolean }): string {
  const listed = report.listed && typeof report.listed === 'object' && 'result' in report.listed
    ? ((report.listed as { result: { tools?: unknown[] } }).result.tools?.length ?? 0)
    : 0;
  const callError = report.called && typeof report.called === 'object' && 'result' in report.called
    ? ((report.called as { result: { isError?: boolean } }).result.isError ?? false)
    : false;
  return [
    'MCP_SMOKE_OK',
    `- tools listed: ${listed}`,
...
async function emitDispatchReceipt(receipt: SignedDispatchReceipt, outFile: string | null, json: boolean, io: CliIo, humanReport: string): Promise<void> {
  const serialized = `${JSON.stringify(receipt, null, 2)}\n`;
  if (outFile !== null) await writeFile(resolve(outFile), serialized, 'utf8');
  if (json) io.out(serialized);
  else io.out(outFile === null ? humanReport : `${humanReport}\nWROTE ${outFile}`);
}

function renderDispatchReport(result: DispatchMutationResult): string {
  return [
...
function isDispatchRejection(error: unknown): boolean {
...
async function emitProviderArtifact(
  artifact: ProviderAdapterBundle | ProviderExecutionPlan | ProviderAuthBindingBundle | CredentialResolutionPlan,
  parsed: { json: boolean; outFile: string | null },
  io: CliIo,
  humanReport: string
): Promise<void> {
  const serialized = `${JSON.stringify(artifact, null, 2)}\n`;
  if (parsed.outFile !== null) await writeFile(resolve(parsed.outFile), serialized, 'utf8');
  if (parsed.json) io.out(serialized);
  else if (parsed.outFile !== null) io.out(`${humanReport}\nWROTE ${parsed.outFile}`);
  else io.out(humanReport);
}

async function emitRuntimeArtifact(
  artifact: AuthorizedExecutionEnvelope,
  parsed: { json: boolean; outFile: string | null },
  io: CliIo,
  humanReport: string
): Promise<void> {
...
async function emitArtifact(
  artifact: SourceInspectionArtifact | CapabilityMapArtifact,
...
function parseValidateArguments(args: string[]): ParsedValidateArguments {
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (argument === '--schemas') {
      ...
      if (!value) throw new Error('--schemas requires a directory.');
...
function parseCompileArguments(args: string[]): ParsedCompileArguments {
  const filePath = args[0];
  if (!filePath || filePath.startsWith('--')) {
...
function parseArtifactArguments(args: string[], command: 'inspect' | 'map' | 'compile' | 'adapters' | 'auth-bindings'): ParsedArtifactArguments {
  const filePath = args[0];
  if (!filePath || filePath.startsWith('--')) {
...
function parseAuthPlanArguments(args: string[]): ParsedAuthPlanArguments {
  const filePath = args[0];
  if (!filePath || filePath.startsWith('--')) throw new Error('ContractBundle path is required for auth-plan.');
  let toolName: string | null = null;
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (argument === '--json') { json = true; continue; }
    if (argument === '--tool') { toolName = requireOptionValue(args, index, '--tool'); index += 1; continue; }
    if (argument === '--credentials') { credentialsFile = requireOptionValue(args, index, '--credentials'); index += 1; continue; }
    if (argument === '--context') { contextFile = requireOptionValue(args, index, '--context'); index += 1; continue; }
    if (argument === '--out') { outFile = requireOptionValue(args, index, '--out'); index += 1; continue; }
    throw new Error(`unknown option ${argument}.`);
  }
  if (toolName === null) throw new Error('--tool is required.');
  if (credentialsFile === null) throw new Error('--credentials is required.');
  if (contextFile === null) throw new Error('--context is required.');
  return { filePath, toolName, credentialsFile, contextFile, json, outFile };
}


function parsePreflightArguments(args: string[]): ParsedPreflightArguments {
  let executionPlanFile: string | null = null;
  let credentialPlanFile: string | null = null;
  let policyFile: string | null = null;
  let approvalFile: string | null = null;
  let budgetFile: string | null = null;
  let trustStoreFile: string | null = null;
  let at: string | null = null;
  let json = false;
  let outFile: string | null = null;
  for (let index = 0; index < args.length; index += 1) {
    const argument = args[index]!;
    if (argument === '--json') { json = true; continue; }
    if (argument === '--execution-plan') { executionPlanFile = requireOptionValue(args, index, '--execution-plan'); index += 1; continue; }
    if (argument === '--credential-plan') { credentialPlanFile = requireOptionValue(args, index, '--credential-plan'); index += 1; continue; }
    if (argument === '--policy') { policyFile = requireOptionValue(args, index, '--policy'); index += 1; continue; }
    if (argument === '--approval') { approvalFile = requireOptionValue(args, index, '--approval'); index += 1; continue; }
    if (argument === '--budget') { budgetFile = requireOptionValue(args, index, '--budget'); index += 1; continue; }
    if (argument === '--trust-store') { trustStoreFile = requireOptionValue(args, index, '--trust-store'); index += 1; continue; }
    if (argument === '--at') { at = requireOptionValue(args, index, '--at'); index += 1; continue; }
    if (argument === '--out') { outFile = requireOptionValue(args, index, '--out'); index += 1; continue; }
    throw new Error(`unknown option ${argument}.`);
  }
  if (executionPlanFile === null) throw new Error('--execution-plan is required.');
  if (credentialPlanFile === null) throw new Error('--credential-plan is required.');
  if (policyFile === null) throw new Error('--policy is required.');
  if (trustStoreFile === null) throw new Error('--trust-store is required.');
  if (at === null) throw new Error('--at is required.');
  return { executionPlanFile, credentialPlanFile, policyFile, approvalFile, budgetFile, trustStoreFile, at, json, outFile };
}

function parsePlanArguments(args: string[]): ParsedPlanArguments {
  const filePath = args[0];
  if (!filePath || filePath.startsWith('--')) throw new Error('ContractBundle path is required for plan.');
  let toolName: string | null = null;
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (argument === '--json') { json = true; continue; }
    if (argument === '--tool') {
      toolName = requireOptionValue(args, index, '--tool'); index += 1; continue;
    }
    if (argument === '--base-url') {
      baseUrl = requireOptionValue(args, index, '--base-url'); index += 1; continue;
    }
    if (argument === '--args') {
      argsFile = requireOptionValue(args, index, '--args'); index += 1; continue;
    }
    if (argument === '--idempotency-key') {
      idempotencyKey = requireOptionValue(args, index, '--idempotency-key'); index += 1; continue;
    }
    if (argument === '--out') {
      outFile = requireOptionValue(args, index, '--out'); index += 1; continue;
    }
    throw new Error(`unknown option ${argument}.`);
  }
  if (toolName === null) throw new Error('--tool is required.');
  if (baseUrl === null) throw new Error('--base-url is required.');
  if (argsFile === null) throw new Error('--args is required.');
  const result: ParsedPlanArguments = { filePath, toolName, baseUrl, argsFile, json, outFile };
...
function renderValidationReport(filePath: string, report: ValidationReport): string {
...
function renderInspectionReport(filePath: string, artifact: SourceInspectionArtifact): string {
...
function renderCapabilityMapReport(filePath: string, artifact: CapabilityMapArtifact): string {
  ...
  const lines = artifact.capabilities.map((capability) =>
    `- ${capability.name} [${capability.risk_class}/${capability.governance.decision}] ← ${capability.source_operation_id}`
  );
  return [header, ...lines].join('\n');
}

function renderCompilationReport(
  filePath: string,
...
function renderAdapterReport(filePath: string, artifact: ProviderAdapterBundle): string {
  return [
...
function renderPlanReport(filePath: string, artifact: ProviderExecutionPlan): string {
  return [
...
function renderAuthBindingReport(filePath: string, artifact: ProviderAuthBindingBundle): string {
  return [
...
function renderAuthPlanReport(filePath: string, artifact: CredentialResolutionPlan): string {
  return [
...
function renderPreflightReport(artifact: AuthorizedExecutionEnvelope): string {
  return [
...
async function runApp(args: string[], io: CliIo): Promise<number> {
  const subcommand = args[0];
  ...
  if (subcommand === 'init') {
    let directory = 'numtema-chatgpt-app'; let force = false; let publicBaseUrl = 'http://127.0.0.1:8788';
    for (let index = 1; index < args.length; index += 1) {
      const argument = args[index]!;
      if (argument === '--force') { force = true; continue; }
      if (argument === '--public-base-url') { publicBaseUrl = requireOptionValue(args, index, '--public-base-url'); index += 1; continue; }
      if (argument.startsWith('--')) { io.error(`USAGE_ERROR: unknown option ${argument}.`); return 2; }
      directory = argument;
    }
    try {
      ...
      return 0;
    } catch (error) { io.error(`APP_INIT_ERROR: ${errorMessage(error)}`); return 2; }
  }
  let configFile: string | null = null; let json = false; let username: string | null = null; let password: string | null = null; let workspace = 'workspace-001'; let toolName: string | undefined; let argsFile: string | undefined;
  for (let index = 1; index < args.length; index += 1) {
    const argument = args[index]!;
    if (argument === '--json') { json = true; continue; }
    if (argument === '--config') { configFile = requireOptionValue(args, index, '--config'); index += 1; continue; }
    if (argument === '--username') { username = requireOptionValue(args, index, '--username'); index += 1; continue; }
    if (argument === '--password') { password = requireOptionValue(args, index, '--password'); index += 1; continue; }
    if (argument === '--workspace') { workspace = requireOptionValue(args, index, '--workspace'); index += 1; continue; }
    if (argument === '--tool') { toolName = requireOptionValue(args, index, '--tool'); index += 1; continue; }
    if (argument === '--args') { argsFile = requireOptionValue(args, index, '--args'); index += 1; continue; }
    io.error(`USAGE_ERROR: unknown option ${argument}.`); return 2;
  }
  if (configFile === null) { io.error('USAGE_ERROR: --config is required.'); return 2; }
  try {
    const assembly = await loadChatGptAppAssembly(configFile);
    if (subcommand === 'inspect') { io.out(json ? JSON.stringify(assembly.summary, null, 2) : renderAppSummary(assembly.summary)); return 0; }
    if (subcommand === 'serve') {
      await new Promise<void>((resolvePromise, reject) => {
        assembly.server.once?.('error', reject);
        assembly.server.listen(assembly.config.server.port, assembly.config.server.host, () => {
          io.error(`CHATGPT_APP_READY ${assembly.summary.mcp_endpoint}`); resolvePromise();
        });
      });
      return await new Promise<number>(() => {});
    }
    if (username === null || password === null) { io.error('USAGE_ERROR: app smoke requires --username and --password.'); return 2; }
    const toolArgs = argsFile === undefined ? undefined : JSON.parse(await readFile(resolve(argsFile), 'utf8')) as Record<string, unknown>;
    ...
    io.out(json ? JSON.stringify(report, null, 2) : `APP_SMOKE_OK ${assembly.summary.mcp_endpoint}\n- OAuth subject: ${String((report.oauth as Record<string, unknown>).subject)}\n- tools/resources: ready\n- refresh token: issued\n- secret material in output: false`);
    return 0;
  } catch (error) { io.error(`APP_${String(subcommand).toUpperCase()}_ERROR: ${errorMessage(error)}`); return 2; }
}


async function runStudio(args: string[], io: CliIo): Promise<number> {
  const subcommand = args[0];
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (argument === '--name') { name = requireOptionValue(args, index, '--name'); index += 1; continue; }
    if (argument === '--force') { force = true; continue; }
    if (argument === '--empty') { empty = true; continue; }
    if (argument === '--json') { json = true; continue; }
    if (argument === '--host') { host = requireOptionValue(args, index, '--host'); index += 1; continue; }
    if (argument === '--port') { port = Number(requireOptionValue(args, index, '--port')); index += 1; continue; }
    io.error(`USAGE_ERROR: unknown option ${argument}.`);
    return 2;
  }
  try {
    ...
    if (!Number.isInteger(port) || port < 0 || port > 65535) throw new Error('STUDIO_PORT_INVALID');
...
function renderStudioProject(project: Awaited<ReturnType<typeof loadStudioProject>>, directory: string): string {
...
function renderAppSummary(summary: ChatGptAppSummary): string {
