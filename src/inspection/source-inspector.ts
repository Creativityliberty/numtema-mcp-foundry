...
export function inspectOpenApi(document: OpenApiDocument, sourceRef: string): SourceInspectionArtifact {
  ...
  return {
    ...
    summary: {
      ...
      high_signal_operation_count: operations.filter((operation) =>
        operation.signals.destructive ||
        operation.signals.financial ||
        operation.signals.external_communication ||
        operation.signals.credential_change
      ).length,
      domains
    },
...
function inspectOperation(
  ...
  if (readOnly) evidence.push(ev('READ_HTTP_METHOD', `${method.toUpperCase()} implies a read-oriented operation.`, 'http_method'));
  if (writes) evidence.push(ev('WRITE_HTTP_METHOD', `${method.toUpperCase()} implies server-side effects.`, 'http_method'));

  const destructive = method === 'delete' || matches(text, ['delete', 'remove', 'purge', 'revoke', 'erase', 'destroy', 'terminate']);
  if (destructive) evidence.push(ev('DESTRUCTIVE_SIGNAL', 'Method or metadata indicates destructive behavior.', method === 'delete' ? 'http_method' : 'operation_metadata'));

  const financial = matches(text, ['refund', 'payment', 'charge', 'payout', 'transfer', 'purchase', 'billing', 'checkout', 'invoice payment']);
  if (financial) evidence.push(ev('FINANCIAL_SIGNAL', 'Operation metadata contains a financial action or resource.', 'operation_metadata'));

  const externalCommunication = matches(text, ['send', 'email', 'sms', 'message', 'notify', 'notification', 'publish', 'broadcast', 'webhook']);
  if (externalCommunication) evidence.push(ev('EXTERNAL_COMMUNICATION_SIGNAL', 'Operation metadata indicates outbound communication or publication.', 'operation_metadata'));

  const credentialChange = matches(text, ['api key', 'api-key', 'token', 'credential', 'secret', 'password', 'permission', 'role', 'access key']);
  if (credentialChange) evidence.push(ev('CREDENTIAL_SIGNAL', 'Operation metadata references credentials or authorization state.', 'operation_metadata'));

  const personalData = matches(text, ['customer', 'patient', 'user', 'contact', 'profile', 'address', 'person', 'employee', 'applicant', 'identity']);
  if (personalData) evidence.push(ev('PERSONAL_DATA_SIGNAL', 'Operation metadata references a person or personal profile.', 'operation_metadata'));
  ...
  if (callbackOrWebhook) evidence.push(ev('CALLBACK_SIGNAL', 'Callbacks or webhooks are declared.', 'callback'));
  ...
  if (asynchronous) evidence.push(ev('ASYNC_SIGNAL', 'Response codes, callbacks, or metadata indicate asynchronous execution.', responseStatuses.includes('202') ? 'response' : 'operation_metadata'));
  ...
  if (upload) evidence.push(ev('UPLOAD_SIGNAL', 'Request media type or operation metadata indicates upload.', requestContentTypes.length > 0 ? 'request_body' : 'operation_metadata'));
  ...
  if (download) evidence.push(ev('DOWNLOAD_SIGNAL', 'Response media type or metadata indicates downloadable content.', responseContentTypes.length > 0 ? 'response' : 'operation_metadata'));

  if (auth.required) evidence.push(ev('AUTH_REQUIRED', `Security schemes required: ${auth.schemes.join(', ') || 'unspecified'}.`, 'security'));
  if (pagination.detected) evidence.push(ev('PAGINATION_SIGNAL', `Pagination parameters detected: ${pagination.parameters.join(', ')}.`, 'parameter'));

  const signals: OperationSignals = {
    read_only: readOnly,
    writes,
    destructive,
    financial,
  ...
  const base: OperationInspection = {
...
function inspectSecuritySchemes(document: OpenApiDocument): SecuritySchemeInspection[] {
...
function collectSchemeScopes(scheme: Record<string, unknown>): string[] {
  if (!isRecord(scheme.flows)) return [];
...
function inspectServers(document: OpenApiDocument): string[] {
...
function inspectParameters(pathItem: Record<string, unknown>, operation: Record<string, unknown>): InspectedParameter[] {
  const rawParameters = [
  ...
  for (const raw of rawParameters) {
    if (!isRecord(raw) || typeof raw.name !== 'string' || typeof raw.in !== 'string') continue;
...
function inspectAuth(document: OpenApiDocument, operation: Record<string, unknown>): OperationInspection['auth'] {
  ...
  for (const requirement of requirements) {
    ...
    if (names.length > 0) hasNonEmptyRequirement = true;
    for (const name of names) {
      schemes.add(name);
      for (const scope of stringArray(requirement[name])) scopes.add(scope);
    }
  }
  return {
...
function inspectPagination(parameters: InspectedParameter[]): OperationInspection['pagination'] {
  const names = parameters
    .filter((parameter) => parameter.location === 'query' && PAGINATION_NAMES.has(parameter.name.toLowerCase()))
    .map((parameter) => parameter.name)
    .sort();
  const lower = names.map((name) => name.toLowerCase());
  const style: OperationInspection['pagination']['style'] = lower.some((name) => ['cursor', 'after', 'before'].includes(name))
    ? 'cursor'
...
function singularize(value: string): string {
  ...
  if (value.toLowerCase().endsWith('sses')) return value.slice(0, -2);
  if (value.toLowerCase().endsWith('s') && !value.toLowerCase().endsWith('ss')) return value.slice(0, -1);
  return value;
}
...
function ev(code: string, message: string, source: InspectionEvidence['source']): InspectionEvidence {
