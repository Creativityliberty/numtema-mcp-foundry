...
import type {
  OAuthAccessTokenClaims, OAuthClient, OAuthGatewayConfig, OAuthTokenResponse, OAuthUser, OAuthPublicJwk
} from './types.js';
...
export interface OAuthService {
  ...
  authorize(input: AuthorizeInput): Promise<{ code: string }>;
  ...
  metadata(): {
    ...
    jwks: { keys: OAuthPublicJwk[] };
  };
}

export function createOAuthService(config: OAuthGatewayConfig): OAuthService {
  ...
  return {
    ...
    async authorize(input) {
      ...
      if (input.codeChallengeMethod !== 'S256') throw new Error('PKCE_METHOD_UNSUPPORTED');
      if (input.codeChallenge.length < 43) throw new Error('PKCE_CHALLENGE_INVALID');
      const user = requireUser(config, input.username, input.password);
      if (!user.workspace_refs.includes(input.workspaceRef)) throw new Error('WORKSPACE_NOT_ALLOWED');
      if (!config.workspaces.some((workspace) => workspace.id === input.workspaceRef)) throw new Error('WORKSPACE_NOT_FOUND');
    ...
    async exchangeAuthorizationCode(input) {
      ...
      const result = await store.mutate((state) => {
        ...
        if (!record) throw new Error('AUTHORIZATION_CODE_INVALID');
        if (record.consumed_at !== null) throw new Error('AUTHORIZATION_CODE_CONSUMED');
        if (Date.parse(record.expires_at) <= Date.parse(now)) throw new Error('AUTHORIZATION_CODE_EXPIRED');
        if (record.client_id !== input.clientId) throw new Error('AUTHORIZATION_CODE_CLIENT_MISMATCH');
        if (record.redirect_uri !== input.redirectUri) throw new Error('REDIRECT_URI_MISMATCH');
        if (record.resource !== input.resource) throw new Error('RESOURCE_MISMATCH');
        if (!verifyPkceS256(input.codeVerifier, record.code_challenge)) throw new Error('PKCE_VERIFICATION_FAILED');
    ...
    async refresh(input) {
      ...
      const result = await store.mutate((state) => {
        ...
        if (record.revoked_at !== null) throw new Error('REFRESH_TOKEN_REVOKED');
        if (Date.parse(record.expires_at) <= Date.parse(now)) throw new Error('REFRESH_TOKEN_EXPIRED');
        if (record.client_id !== input.clientId) throw new Error('REFRESH_TOKEN_CLIENT_MISMATCH');
        if (record.resource !== input.resource) throw new Error('RESOURCE_MISMATCH');
    ...
    async registerClient(input, now = new Date().toISOString()) {
      if (!config.allow_dynamic_client_registration) throw new Error('DYNAMIC_CLIENT_REGISTRATION_DISABLED');
      if (!input.client_name || !Array.isArray(input.redirect_uris) || input.redirect_uris.length === 0) throw new Error('CLIENT_METADATA_INVALID');
      for (const redirect of input.redirect_uris) validateRedirectUriSecurity(redirect);
      const grants = input.grant_types ?? ['authorization_code', 'refresh_token'];
      if (grants.some((grant) => grant !== 'authorization_code' && grant !== 'refresh_token')) throw new Error('GRANT_TYPE_UNSUPPORTED');
      const responses = input.response_types ?? ['code'];
      if (responses.length !== 1 || responses[0] !== 'code') throw new Error('RESPONSE_TYPE_UNSUPPORTED');
      if ((input.token_endpoint_auth_method ?? 'none') !== 'none') throw new Error('CLIENT_AUTH_METHOD_UNSUPPORTED');
      const client: OAuthClient = {
        client_id: `client_${randomUUID()}`,
        client_name: input.client_name,
        redirect_uris: [...new Set(input.redirect_uris)].sort(),
        grant_types: [...new Set(grants)] as ('authorization_code' | 'refresh_token')[],
    ...
    async revoke(token, now = new Date().toISOString()) {
      if (token.includes('.')) {
        ...
        await store.mutate((state) => { if (!state.revoked_access_jti.includes(claims.jti)) state.revoked_access_jti.push(claims.jti); });
        return;
      }
      await store.mutate((state) => {
...
async function requireClient(store: OAuthStore, clientId: string): Promise<OAuthClient> {
  const client = (await store.read()).clients.find((entry) => entry.client_id === clientId);
  if (!client) throw new Error('CLIENT_NOT_FOUND');
  return client;
}

function requireUser(config: OAuthGatewayConfig, username: string, password: string): OAuthUser {
...
export function isMatchingRedirectUri(requested: string, allowed: string[]): boolean {
  ...
  try {
    ...
    if (u1.hostname === 'chatgpt.com' && u1.pathname.endsWith('/oauth/callback')) return true;
...
function validateScopes(requested: string[], supported: string[], allowed: string[]): string[] {
  ...
  for (const scope of scopes) {
    if (!supported.includes(scope)) throw new Error(`SCOPE_UNSUPPORTED: ${scope}`);
    if (!allowed.includes(scope)) throw new Error(`SCOPE_NOT_ALLOWED: ${scope}`);
  }
  return scopes;
}
function validateRedirectUriSecurity(value: string): void {
  let url: URL;
  try { url = new URL(value); } catch { throw new Error('REDIRECT_URI_INVALID'); }
  const localhost = url.hostname === 'localhost' || url.hostname === '127.0.0.1' || url.hostname === '[::1]';
  if (url.protocol !== 'https:' && !(url.protocol === 'http:' && localhost)) throw new Error('REDIRECT_URI_INSECURE');
  if (url.hash) throw new Error('REDIRECT_URI_FRAGMENT_FORBIDDEN');
}
function issueTokens(config: OAuthGatewayConfig, subject: string, clientId: string, workspaceRef: string, scopes: string[], refreshToken: string, now: string): OAuthTokenResponse {
  return {
...
function addSeconds(at: string, seconds: number): string {
  return new Date(Date.parse(at) + seconds * 1000).toISOString();
}
function prune(state: { authorization_codes: { expires_at: string }[]; refresh_tokens: { expires_at: string }[] }, now: string): void {
...
function validateConfig(config: OAuthGatewayConfig): void {
  const issuer = new URL(config.issuer);
  const resource = new URL(config.resource);
  if (issuer.protocol !== 'https:' && issuer.hostname !== 'localhost' && issuer.hostname !== '127.0.0.1') throw new Error('OAUTH_ISSUER_HTTPS_REQUIRED');
  if (resource.hash) throw new Error('OAUTH_RESOURCE_FRAGMENT_FORBIDDEN');
  if (!config.scopes_supported.every((scope) => typeof scope === 'string' && scope.length > 0)) throw new Error('OAUTH_SCOPES_INVALID');
  for (const scope of config.baseline_scopes) if (!config.scopes_supported.includes(scope)) throw new Error('OAUTH_BASELINE_SCOPE_UNSUPPORTED');
}
