...
export function verifyAccessToken(token, options) {
    ...
    if (!publicKeyPem)
        throw new Error('TOKEN_KEY_UNKNOWN');
    ...
    if (claims.iss !== options.issuer)
        throw new Error('TOKEN_ISSUER_MISMATCH');
...
function decodeJson(value) {
    try {
        return JSON.parse(Buffer.from(fromBase64Url(value), 'base64').toString('utf8'));
    }
    catch {
        throw new Error('TOKEN_JSON_INVALID');
    }
}
