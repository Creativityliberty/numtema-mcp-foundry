...
export function extractOperationSchema(document, pathItem, operation) {
    const unresolved = new Set();
    const resolve = (value) => resolveLocalRefs(document, value, unresolved, new Set());
    ...
    if (requestBody)
        envelope.request_body = requestBody;
    return envelope;
}
function mergeParameters(pathParameters, operationParameters, resolve) {
    ...
    for (const raw of [...array(pathParameters), ...array(operationParameters)]) {
        ...
        if (typeof value.style === 'string')
            parameter.style = value.style;
        if (typeof value.explode === 'boolean')
            parameter.explode = value.explode;
...
function inspectRequestBody(value) {
    if (!isRecord(value))
        return undefined;
    const content = inspectContent(value.content);
    if (content.length === 0)
        return undefined;
    const body = { required: value.required === true, content };
    if (typeof value.description === 'string')
        body.description = value.description;
    return body;
}
function inspectResponses(value) {
    if (!isRecord(value))
        return [];
...
function resolveLocalRefs(document, value, unresolved, stack) {
    if (Array.isArray(value))
        return value.map((entry) => resolveLocalRefs(document, entry, unresolved, stack));
    if (!isRecord(value))
        return value;
    ...
    for (const key of Object.keys(value).sort())
        result[key] = resolveLocalRefs(document, value[key], unresolved, stack);
    return result;
}
...
function paginationFromParameters(parameters) {
...
function collectPropertyNames(schema, result, depth) {
    ...
    if (isRecord(schema.items))
        collectPropertyNames(schema.items, result, depth + 1);
    if (Array.isArray(schema.oneOf))
        schema.oneOf.forEach((entry) => collectPropertyNames(entry, result, depth + 1));
    if (Array.isArray(schema.anyOf))
        schema.anyOf.forEach((entry) => collectPropertyNames(entry, result, depth + 1));
}
function isBinary(mediaType, schema) {
    if (mediaType === 'application/octet-stream' || mediaType.startsWith('image/') || mediaType.startsWith('video/') || mediaType.startsWith('audio/'))
        return true;
    return containsBinarySchema(schema);
}
function containsBinarySchema(value) {
    if (Array.isArray(value))
        return value.some(containsBinarySchema);
    if (!isRecord(value))
        return false;
    if (value.format === 'binary' || value.contentEncoding === 'base64')
        return true;
...
function cloneWithoutRef(value) {
    const result = {};
...
function statusCompare(left, right) {
    if (left === 'default')
        return 1;
    if (right === 'default')
        return -1;
...
//# sourceMappingURL=openapi-schema-extractor.js.map