...
export async function initializeDispatchLedger(input) {
    ...
    return withLedgerLock(directory, 5_000, async () => {
        ...
        const metadata = { ...base, integrity: { algorithm: 'sha256', digest: sha256ArtifactPayload(base) } };
...
export async function reserveDispatch(input) {
    ...
    return withLedgerLock(directory, timeout, async () => {
        ...
        if (sameKey !== undefined) {
            if (sameKey.creation.envelope_digest !== envelopeDigest)
                throw new Error('IDEMPOTENCY_CONFLICT: key is already bound to a different authorized envelope.');
            if (sameKey.state !== 'reserved')
                throw new Error('AUTHORIZATION_ALREADY_TERMINAL: exact authorization already reached a terminal dispatch state.');
        ...
        for (const record of state.reservations.values()) {
            if (record.creation.execution_nonce === envelope.execution_nonce)
                throw new Error('NONCE_ALREADY_CONSUMED: execution nonce is permanently single-use.');
            ...
            if (record.creation.authorization_id === envelope.authorization_id)
                throw new Error('AUTHORIZATION_ALREADY_RESERVED: authorization id already exists in the ledger.');
        }
        const budget = budgetFromEnvelope(envelope);
        if (budget !== null)
            ensureBudgetAvailable(state, budget);
        ...
        const record = { creation: event, latest: event, state: 'reserved' };
...
export async function commitDispatch(input) {
    ...
    return withLedgerLock(directory, input.lockTimeoutMs ?? 5_000, async () => {
        ...
        if (record.state !== 'reserved')
            throw new Error('INVALID_DISPATCH_TRANSITION: only a reserved authorization may be dispatched.');
        ...
        await appendEvent(directory, event);
        const updated = { creation: record.creation, latest: event, state: 'dispatched' };
...
export async function releaseDispatch(input) {
    ...
    return withLedgerLock(directory, input.lockTimeoutMs ?? 5_000, async () => {
        ...
        if (record.state === 'released') {
            if (record.latest.reason !== input.reason)
                throw new Error('INVALID_DISPATCH_TRANSITION: reservation was released for a different reason.');
            const reservation = reservationFromRecord(state.metadata.ledger_id, record, true);
            return { reservation, receipt: signReceipt(record.latest, reservation, 'reserved_to_released', true, input), replayed: true };
        }
        if (record.state !== 'reserved')
            throw new Error('INVALID_DISPATCH_TRANSITION: only a reserved authorization may be released.');
        ...
        await appendEvent(directory, event);
        const updated = { creation: record.creation, latest: event, state: 'released' };
        const reservation = reservationFromRecord(state.metadata.ledger_id, updated, false);
        return { reservation, receipt: signReceipt(event, reservation, 'reserved_to_released', false, input), replayed: false };
    });
}
export async function readDispatchLedgerSnapshot(directory, generatedAt) {
...
export function verifyDispatchReceipt(receipt, trustStore, at) {
    return verifySignedArtifact(receipt, trustStore, 'dispatch', at);
}
async function loadLedgerState(directory) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (parsed.artifact_type !== 'dispatch_ledger_event' || parsed.artifact_version !== '0.9')
            throw new Error(`LEDGER_CORRUPT: invalid event artifact at line ${index + 1}.`);
        if (parsed.ledger_id !== metadata.ledger_id)
            throw new Error(`LEDGER_CORRUPT: event ledger id mismatch at line ${index + 1}.`);
        if (parsed.sequence !== index + 1)
            throw new Error(`LEDGER_CORRUPT: non-contiguous sequence at line ${index + 1}.`);
        if (parsed.previous_event_digest !== previousDigest)
            throw new Error(`LEDGER_CORRUPT: previous digest mismatch at line ${index + 1}.`);
    ...
    for (const event of events) {
        ...
        if (event.event_type === 'reservation_created') {
            if (existing !== undefined)
                throw new Error(`LEDGER_CORRUPT: duplicate reservation ${event.reservation_id}.`);
            reservations.set(event.reservation_id, { creation: event, latest: event, state: 'reserved' });
            continue;
        }
        if (existing === undefined)
            throw new Error(`LEDGER_CORRUPT: transition without reservation ${event.reservation_id}.`);
        if (existing.state !== 'reserved')
            throw new Error(`LEDGER_CORRUPT: transition from terminal state for ${event.reservation_id}.`);
...
async function loadMetadata(directory) {
    const metadata = JSON.parse(await readFile(join(directory, METADATA_FILE), 'utf8'));
    if (metadata.artifact_type !== 'dispatch_ledger_metadata' || metadata.artifact_version !== '0.9')
        throw new Error('INVALID_LEDGER_METADATA: unsupported metadata artifact.');
    if (metadata.integrity.algorithm !== 'sha256' || metadata.integrity.digest !== sha256ArtifactPayload(metadata))
        throw new Error('INVALID_LEDGER_METADATA: digest mismatch.');
    return metadata;
}
function validateAuthorizedEnvelope(envelope, at) {
    assertDateTime(at, 'INVALID_RESERVATION_TIME');
    if (envelope.artifact_type !== 'authorized_execution_envelope' || envelope.artifact_version !== '0.8')
        throw new Error('INVALID_AUTHORIZATION_ENVELOPE: unsupported artifact.');
    if (!envelope.dispatch_permitted || !envelope.dry_run || envelope.network_executed)
        throw new Error('DISPATCH_NOT_AUTHORIZED: envelope does not permit dispatch.');
    if (envelope.integrity.algorithm !== 'sha256' || envelope.integrity.digest !== sha256ArtifactPayload(envelope))
        throw new Error('AUTHORIZATION_INTEGRITY_MISMATCH: authorized envelope digest is invalid.');
    if (envelope.expires_at !== null && Date.parse(at) >= Date.parse(envelope.expires_at))
        throw new Error('AUTHORIZATION_EXPIRED: authorized envelope has expired.');
    if (containsSecretMaterial(envelope))
        throw new Error('SECRET_MATERIAL_DETECTED: authorized envelope contains forbidden secret material.');
    if (envelope.execution_plan.idempotency.mode === 'required' && !envelope.execution_plan.idempotency.key)
        throw new Error('IDEMPOTENCY_KEY_REQUIRED: provider plan requires an idempotency key.');
...
function ensureBudgetAvailable(state, budget) {
    let active = 0;
    for (const record of state.reservations.values()) {
        if (record.state === 'released')
            continue;
        const candidate = record.creation.budget;
        if (candidate?.budget_ref === budget.budget_ref && candidate.currency === budget.currency)
            active += candidate.amount;
    }
    const available = budget.remaining_before - active;
    if (budget.amount > available)
        throw new Error(`BUDGET_RESERVATION_EXCEEDED: requested ${budget.amount} ${budget.currency}, only ${available} remains after durable reservations.`);
}
function budgetFromEnvelope(envelope) {
    if (!envelope.budget.required)
        return null;
    return {
...
function createTransitionEvent(state, record, eventType, stateAfter, reason, occurredAt) {
    return createEvent({
        ledgerId: state.metadata.ledger_id,
...
function reservationFromRecord(ledgerId, record, replayed) {
    const base = {
...
function signReceipt(event, reservation, transition, replayed, signer) {
...
function buildBudgetTotals(reservations) {
    ...
    for (const reservation of reservations) {
        ...
        if (reservation.state === 'released')
            total.released_amount += budget.amount;
        else
            total.active_amount += budget.amount;
        if (reservation.state === 'dispatched')
            total.dispatched_amount += budget.amount;
...
async function withLedgerLock(directory, timeoutMs, fn) {
    ...
    while (true) {
        try {
            ...
            if (Date.now() - started >= timeoutMs)
                throw new Error('LEDGER_LOCK_TIMEOUT: could not acquire the durable dispatch lock.');
            await sleep(10);
        }
    }
    try {
...
function containsSecretMaterial(value, key = '') {
    ...
    for (const [childKey, child] of Object.entries(value)) {
        ...
        if (/^(access_token|refresh_token|api_key|password|client_secret|private_key|secret_locator|secret)$/.test(normalized) && child !== null && child !== false && child !== '')
            return true;
        if (containsSecretMaterial(child, normalized))
            return true;
    }
    return false;
}
function assertDateTime(value, code) {
    if (Number.isNaN(Date.parse(value)))
        throw new Error(`${code}: value must be RFC 3339.`);
}
