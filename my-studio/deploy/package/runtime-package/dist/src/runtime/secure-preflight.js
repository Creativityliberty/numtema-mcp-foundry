...
export function authorizeExecution(input) {
    ...
    if (Number.isNaN(atMs))
        errors.push({ code: 'INVALID_PREFLIGHT_TIME', message: 'Preflight timestamp must be RFC 3339.' });
    ...
    if (input.approval !== undefined)
        expiryCandidates.push(input.approval.expires_at);
    if (input.budget !== undefined)
        expiryCandidates.push(input.budget.expires_at);
    ...
    const base = {
        ...
        warnings,
        errors,
        redaction: { secret_material_included: false, credential_handle_only: true },
        source_digests: sourceDigests
    };
    return { ...base, integrity: { algorithm: 'sha256', digest: sha256(base) } };
}
function validateApproval(approval, required, policy, binding, trustStore, at, atMs) {
    const checks = [];
    const errors = [];
    if (!required) {
...
function validateBudget(budget, required, policy, binding, trustStore, at, atMs) {
    const checks = [];
    const errors = [];
    if (!required) {
...
function verifyPlanIntegrity(label, plan, checks, errors) {
...
function appendVerification(prefix, verificationErrors, checks, errors) {
...
function check(condition, code, passMessage, failMessage, checks, errors, errorCode = code) {
    checks.push({ code, status: condition ? 'pass' : 'fail', message: condition ? passMessage : failMessage });
    if (!condition)
        errors.push({ code: errorCode, message: failMessage });
}
function checkTimeWindow(prefix, issuedAt, expiresAt, atMs, checks, errors) {
    ...
    if (!validFormat || Number.isNaN(atMs))
        return;
...
function tenantBindingsEqual(left, right) {
...
function containsSecretMaterial(value, key = '') {
    ...
    if ((normalizedKey === 'secret_material_included' || normalizedKey === 'secret_locator_included') && value === true)
        return true;
    ...
    if (Array.isArray(value))
        return value.some((child) => containsSecretMaterial(child));
    if (typeof value === 'object' && value !== null)
        return Object.entries(value).some(([childKey, child]) => containsSecretMaterial(child, childKey));
    return false;
}
//# sourceMappingURL=secure-preflight.js.map