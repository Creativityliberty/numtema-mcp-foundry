...
export async function createStudioProject(directory, options = {}) {
    ...
    await writeJsonAtomic(root, project.paths.overrides, []);
...
export async function loadStudioProject(directory) {
    const root = resolve(directory);
    const parsed = JSON.parse(await readFile(join(root, PROJECT_FILE), 'utf8'));
    if (parsed.artifact_type !== 'studio_project' || parsed.artifact_version !== '1.2')
        throw new Error('STUDIO_PROJECT_INVALID: unsupported project descriptor.');
    validateProjectPaths(root, parsed);
    return parsed;
}
export async function saveStudioProject(directory, project) {
    const root = resolve(directory);
...
export async function loadStudioOverrides(directory, project) {
    const root = resolve(directory);
    const selected = project ?? await loadStudioProject(root);
    const parsed = JSON.parse(await readFile(resolveWithin(root, selected.paths.overrides), 'utf8'));
    if (!Array.isArray(parsed))
        throw new Error('STUDIO_OVERRIDES_INVALID: expected an array.');
    return parsed;
}
export async function saveStudioOverrides(directory, overrides) {
    const root = resolve(directory);
    const project = await loadStudioProject(root);
    await writeJsonAtomic(root, project.paths.overrides, overrides);
}
export async function writeProjectJson(directory, relativePath, value) {
    const root = resolve(directory);
...
export function resolveStudioPath(directory, relativePath) {
    return resolveWithin(resolve(directory), relativePath);
}
export async function importStudioSource(directory, filename, content) {
    if (content.length > 2_000_000)
        throw new Error('STUDIO_SOURCE_TOO_LARGE: source exceeds 2 MB.');
...
function validateProjectPaths(root, project) {
    for (const value of [project.source.file, ...Object.values(project.paths)])
        resolveWithin(root, value);
}
function resolveWithin(root, candidate) {
    const absolute = resolve(root, candidate);
    const rel = relative(root, absolute);
    if (rel.startsWith('..') || rel === '..')
        throw new Error(`STUDIO_PATH_TRAVERSAL: ${candidate}`);
    return absolute;
}
...
function normalizeName(value) {
    ...
    if (normalized.length === 0)
        throw new Error('STUDIO_PROJECT_NAME_INVALID: name cannot be empty.');
    return normalized;
}
