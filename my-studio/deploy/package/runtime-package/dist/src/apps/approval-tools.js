        ...
        async callTool(name, args, context) {
            try {
                ...
                if (name === 'foundry_approval_prepare') {
                    ...
                    if (!tool || !tool.approval_ref)
                        throw new Error('APPROVAL_TARGET_NOT_GOVERNED');
                    const adapter = adapters.get(targetName);
                    if (!adapter)
                        throw new Error('APPROVAL_TARGET_ADAPTER_MISSING');
                    const contract = approvals.get(tool.approval_ref);
                    if (!contract)
                        throw new Error('APPROVAL_CONTRACT_NOT_FOUND');
...
function stringArg(args, key) { const value = args[key]; if (typeof value !== 'string' || value.length === 0)
    throw new Error(`INVALID_${key.toUpperCase()}`); return value; }
function recordArg(args, key) { const value = args[key]; if (!isRecord(value))
    throw new Error(`INVALID_${key.toUpperCase()}`); return value; }
