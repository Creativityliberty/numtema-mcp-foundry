...
export async function initializeChatGptApp(directory, force = false, publicBaseUrl = 'http://127.0.0.1:8788') {
    ...
    const contracts = JSON.parse(await readFile(join(target, 'contract-bundle.auth.generated.json'), 'utf8'));
...
async function writePrivate(path, value) { await writeFile(path, value, 'utf8'); await chmod(path, 0o600); }
function keyPair() { const pair = generateKeyPairSync('ed25519'); return { publicKey: String(pair.publicKey.export({ type: 'spki', format: 'pem' })), privateKey: String(pair.privateKey.export({ type: 'pkcs8', format: 'pem' })) }; }
function randomPassword() { return `Nf!${randomBytes(12).toString('hex')}`; }
function renderReadme(username, password, base) { return `# Nümtema ChatGPT App Demo\n\nIdentifiants de développement générés une seule fois :\n\n- Username: \`${username}\`\n- Password: \`${password}\`\n\nNe réutilisez pas ces identifiants en production.\n\n## Démarrer\n\n\`\`\`bash\nCUSTOMER_API_TOKEN=demo-token node mock-provider.mjs\nCUSTOMER_API_TOKEN=demo-token foundry app serve --config chatgpt-app-config.json\n\`\`\`\n\nMCP endpoint: ${base}/mcp\nOAuth metadata: ${base}/.well-known/oauth-protected-resource\n`; }
