...
export async function initializeMcpRuntime(directory, force = false) {
    const target = resolve(directory);
    if (await exists(target)) {
        const entries = await readdir(target);
        if (entries.length > 0 && !force)
            throw new Error(`MCP_INIT_REFUSED: ${target} is not empty. Use --force to replace generated files.`);
    }
    await mkdir(target, { recursive: true });
    for (const name of ARTIFACTS)
        await copyFile(resolvePackageAsset('examples', name), join(target, name));
