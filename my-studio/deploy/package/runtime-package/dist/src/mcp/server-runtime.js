    ...
    return {
        async callTool(name, args, meta = {}, requestContext) {
            ...
            try {
                ...
                if (!envelope.dispatch_permitted)
                    return preflightFailure(name, envelope.errors);
...
function requireAdapter(adapters, name) {
    ...
    if (adapter === undefined)
        throw new Error(`ADAPTER_NOT_FOUND: no provider adapter exists for ${name}.`);
    return adapter;
}
function requirePolicy(policies, tool) {
    if (tool.policy_ref === undefined)
        throw new Error(`POLICY_REQUIRED: tool ${tool.name} has no PolicyContract.`);
    const policy = policies.get(tool.policy_ref);
    if (policy === undefined)
        throw new Error(`POLICY_NOT_FOUND: ${tool.policy_ref}.`);
    return policy;
}
function validateArguments(tool, args) {
    const issues = validateValueAgainstSchema(tool.input_schema, args, 'mcp_tool_arguments', tool.id);
    if (issues.length > 0)
        throw new Error(`INVALID_TOOL_ARGUMENTS: ${issues.map((issue) => `${issue.path || '/'} ${issue.message}`).join('; ')}`);
}
function resolveIdempotencyKey(adapter, meta, at, args) {
    const supplied = meta.idempotencyKey;
    if (supplied !== undefined) {
        if (typeof supplied !== 'string' || supplied.length === 0)
            throw new Error('INVALID_IDEMPOTENCY_KEY: _meta.idempotencyKey must be a non-empty string.');
        return supplied;
    }
...
function createNoCredentialPlan(adapter, context) {
    const base = {
        ...
        checks: [{ code: 'AUTH_NOT_REQUIRED', status: 'not_applicable', message: 'Provider adapter does not require authentication.' }],
        ...
        injection_envelope: { strategy: 'none', location: 'runtime', name: null, prefix: null, signing_algorithm: null, credential_handle: null, secret_material_included: false },
        ready: true, warnings: [], errors: []
    };
    return { ...base, integrity: { algorithm: 'sha256', digest: sha256(base) } };
}
function preflightFailure(name, errors) {
...
function providerResult(name, normalized, executionReceipt, reservationReceipt, dispatchReceipt) {
    if (normalized.ok) {
        return {
...
function asStructuredContent(value) {
    ...
    if (value instanceof Uint8Array)
        return { binary: true, byte_length: value.byteLength, digest: sha256([...value]) };
    return { data: value };
}
