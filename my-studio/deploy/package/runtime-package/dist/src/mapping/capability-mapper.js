...
export function mapCapabilities(inspection) {
...
function mapOperation(operation, usedNames) {
...
function classifyRisk(operation) {
    if (operation.signals.credential_change && operation.signals.destructive)
        return 'R5';
    if (operation.signals.financial)
        return 'R4';
    if (operation.signals.destructive || operation.signals.credential_change || operation.signals.external_communication)
        return 'R3';
    if (operation.signals.writes)
        return 'R2';
    if (operation.signals.personal_data || operation.auth.required)
        return 'R1';
    return 'R0';
}
function governanceFor(risk, operation) {
    const reasons = [`Classified ${risk} from source inspection signals.`];
    let decision;
    let approvalMode;
    switch (risk) {
        case 'R5':
...
function fallbackName(operation) {
...
function uniqueName(baseName, method, usedNames) {
...
function buildDomains(capabilities) {
...
function detectWorkflowHints(operations) {
    const hints = [];
    ...
    for (const [domain, group] of byDomain) {
        for (const operation of group) {
            if (operation.signals.asynchronous && operation.signals.writes) {
                ...
                const status = group.find((candidate) => candidate.method === 'get' &&
            ...
            if (operation.signals.upload) {
                const confirm = group.find((candidate) => candidate.operation_id !== operation.operation_id && semanticTokens(candidate.operation_id).includes('confirm'));
                if (confirm) {
                    hints.push({
...
function singularizeWords(words) {
    return words.map(singularize);
}
function singularize(value) {
    if (value === 'status' || value === 'news')
        return value;
