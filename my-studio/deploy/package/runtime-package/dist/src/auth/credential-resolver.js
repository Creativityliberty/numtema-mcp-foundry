...
export function resolveCredentialPlan(binding, adapter, catalog, context) {
    const checks = [];
    const errors = [];
    const warnings = [];
    ...
    if (selected === null)
        errors.push({ code: 'NO_MATCHING_CREDENTIAL', message: 'No active, unexpired credential matches the requested subject, client, workspace, provider, and provider account.' });
    if (selected !== null && missingScopes.length > 0)
        errors.push({ code: 'INSUFFICIENT_SCOPES', message: `Missing scopes: ${missingScopes.join(', ')}.` });
    if (selected !== null && !audienceMatched)
        errors.push({ code: 'AUDIENCE_MISMATCH', message: `Credential is not valid for audience ${expectedAudience ?? 'unknown'}.` });
    if (binding.auth_mode === 'host_managed')
        warnings.push({ code: 'HOST_MANAGED_RESOLUTION', message: 'The host must resolve and inject credential material at execution time.' });
...
function matchesDimension(dimension, binding, accountValue, contextValue) {
    if (!binding.binding_dimensions.includes(dimension))
        return true;
    return accountValue === contextValue;
}
function isExpired(account, requestedAt) {
